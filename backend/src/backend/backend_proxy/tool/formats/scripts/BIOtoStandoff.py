...
def BIOtoStandoff(text:str,bio:str):
    tokenIdx = 0
    bioIdx = "1"
    ...
    for s in so:
        ...
        if len(splitted) >=4:
            response['entities'].append([splitted[0],splitted[1],[[int(splitted[2]),int(splitted[3])]]])
        
    
    return response

# text = '''Helmut Kohl'ün aradaki farkı kapatmasında da kararsız seçmenlerin bir bölümünü merkez sağa çekmesinin rol oynadığı ileri sürüldü'''
...
#     BIOtoStandoff(text=text,bio=bio)